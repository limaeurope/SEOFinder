...
static void		EditReport (const GS::Array<API_Neig>& items, const GS::Array<API_Neig>& items_save)
{
	...
	for (UIndex i = 0; i < items.GetSize (); i++) {
		...
		if (items[i].guid == APINULLGuid) {
			WriteReport ("[%d]  %s GUID:%s - refused", i+1, UniStringToConstUTF8String (ElemID_To_Name (typeID)),
			             									UniStringToConstCString (APIGuidToString (items_save[i].guid)));
		} else {
			WriteReport ("[%d]  %s GUID:%s -> GUID:%s", i+1, UniStringToConstUTF8String (ElemID_To_Name (typeID)),
			             									 UniStringToConstCString (APIGuidToString (items_save[i].guid)),
			             									 UniStringToConstCString (APIGuidToString (items[i].guid)));
		}
	}
}		// EditReport
