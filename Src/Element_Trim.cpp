...
void Do_Trim_ElementsWith (void)
{
	...
	if (!guid_ElementsToTrim.IsEmpty ()) {
		...
		if (!ClickAnElem ("Click on a Roof or Shell to Trim with!", API_ZombieElemID, nullptr, &typeID, &guid_Element)) {
...
void Do_Trim_GetTrimmingElements (void)
{
	...
	if (!ClickAnElem ("Click on a Roof or Shell element to get the trimming elements!", API_ZombieElemID, nullptr, &typeID, &guid_Element)) {
		WriteReport_Alert ("No element was clicked.");
		return;
	}
...
void Do_Merge_GetMergedElements (void)
{
	...
	if (!ClickAnElem ("Click on an element to get the merged elements!", API_ZombieElemID, nullptr, &typeID, &guid_Element)) {
		WriteReport_Alert ("No element was clicked.");
		return;
	}
