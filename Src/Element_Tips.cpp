...
void		Do_AvoidFlashing (short mode)
{
	...
	err = ACAPI_Element_GetConnectedElements (guid, API_WindowID, &wallWindows);
	if (err == NoError)
		err = ACAPI_Element_GetConnectedElements (guid, API_DoorID, &wallDoors);
	...
	if (mode == 3) {		// best way: do the operation in one API call
		...
		for (const API_Guid& guid : elemGuids) {
			...
			if (err == NoError)
				WriteReport ("  GUID:%s  deleted", UniStringToConstCString (APIGuidToString (guid)));
			else
				WriteReport ("  GUID:%s  ERROR: %d", UniStringToConstCString (APIGuidToString (guid)), err);
		}
	}
...
void		Do_GetFirstWindowPosition (void)
{
	...
	if (!ClickAnElem ("Click an wall to get the direction of the first opening", API_WallID, nullptr, &typeID, &wallGuid)) {
		WriteReport_Alert ("No wall was clicked");
		return;
	}
	...
	err = ACAPI_Element_Get (&element);
	if (err == NoError)
		err = ACAPI_Element_GetConnectedElements (wallGuid, API_WindowID, &connWinds);
	...
	API_Tranmat tm;
	if (ACAPI_Goodies (APIAny_GetOpeningTransformationID, &connWinds[0], &tm) == NoError) {
