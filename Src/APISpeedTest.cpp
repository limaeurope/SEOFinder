...
public:
	...
	~MeasureTime ()
	{
		...
		if (mNumberOfOperations > 0)
			WriteReportLine (GS::UniString::Printf ("%6u operation:  %12.6f seconds.", 1, elapsedSeconds / mNumberOfOperations));
		WriteReportLine ("");
	}
...
static void		CreateSomeElements (const GS::UniString& measurementText, API_ElemTypeID elemType, CreateAtReferencePointProc createProc)
{
	...
	ACAPI_Element_GetDefaults (&defaultElem, nullptr);
...
void	Do_Speed_DeleteAllOneByOne (void)
{
...

	{
		...
		for (auto it = allElements.Enumerate (); it != nullptr; ++it) {
			ACAPI_Element_Delete ({ *it });
		}
	}
}
...
void	Do_Speed_DeleteAllWithOneCall (void)
{
...

	{
		...
		ACAPI_Element_Delete (allElements);
	}
}
