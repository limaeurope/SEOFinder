...
void	Do_CreateLabel_Associative (void)
{
	...
	API_Guid		guid;
	API_Coord3D		begC3D;
	if (!ClickAnElem ("Click the element to assign the Label to. The clicked point will be the \"begC\" of the Label.", API_ZombieElemID, nullptr, &typeID, &guid, &begC3D)) {
...
void	Do_CreatePerspCam (void)
{
	...
	if (!ClickAnElem ("Click a camera to insert a new camera after or click a place to create a new path/camera", API_ZombieElemID, nullptr, &typeID, &guid, &pos3D)) {
		WriteReport_Alert ("No element was clicked");
		return;
	}
...
void	Do_CreateSkylight (void)
{
	...
	if (!ClickAnElem ("Click on a roof or a shell to place a skylight", API_ZombieElemID, nullptr, &typeID, &guid, &c3) ||
	...
	for (Int32 i = 0; i < selectionInfo.sel_nElemEdit; i++) {
		...
		if (err != NoError) {
			ACAPI_WriteReport ("Element type %s, failed to add to drawing: %d\nOnly 2D elements can be placed on drawing.", true,
			                      UniStringToConstUTF8String (ElemID_To_Name (element.header.typeID)),
			                      (int) err);
		}
...
void		Do_PickupElem (void)
{
	...
	if (!ClickAnElem ("Click an element", API_ZombieElemID, nullptr, &typeID, &guid)) {
		WriteReport_Alert ("No element was clicked");
		return;
	}
...
void		Do_ExplodeElem (void)
{
	...
	if (!ClickAnElem ("Click an elem to explode", API_ZombieElemID, nullptr, &typeID, &guid)) {
		WriteReport_Alert ("No element was clicked");
		return;
	}
...
void		Do_DumpElem (API_Guid& renFiltGuid)
{
	...
	if (!ClickAnElem ("Click an elem to get info for", API_ZombieElemID, nullptr, &typeID, &guid)) {
		WriteReport_Alert ("No element was clicked");
		return;
	}
	...
	WriteReport ("Dump: %s GUID:%s", UniStringToConstUTF8String (ElemID_To_Name (typeID)),
...
void		Do_SetElemCategories (bool changeDefaults)
{
	...
	if (!ClickAnElem ("Click an elem to set categories", API_ZombieElemID, nullptr, &typeID, &guid)) {
		WriteReport_Alert ("No element was clicked");
		return;
	}
	...
	WriteReport ("Clicked element: %s GUID:%s", UniStringToConstUTF8String (ElemID_To_Name (typeID)),
	             								UniStringToConstCString (APIGuidToString (element.header.guid)));
		...
		if (category.categoryID == API_ElemCategory_StructuralFunction || category.categoryID == API_ElemCategory_Position) {
			...
			if (!categoryValueList.IsEmpty ()) {
				...
				if (changeDefaults) {
					...
					err = ACAPI_Element_SetCategoryValueDefault (element.header.typeID, element.header.variationID, category, elemCategoryValue);
					if (err != NoError)
...
void Do_DumpCollisions (GS::Array<GS::Pair<API_CollisionElem, API_CollisionElem>>& collisions)
{
	...
	for (auto pair : collisions.AsConst ()) {
		API_Element		elem1;
		BNZeroMemory (&elem1, sizeof (API_Element));
		...
		if (err == NoError) {
			WriteReport ("\n%d. Collision: ", ind);
			WriteReport ("1. GUID: %s, type: %s", APIGuidToString (elem1.header.guid).ToCStr ().Get (), UniStringToConstUTF8String (ElemID_To_Name (elem1.header.typeID)));
			WriteReport ("2. GUID: %s, type: %s", APIGuidToString (elem2.header.guid).ToCStr ().Get (), UniStringToConstUTF8String (ElemID_To_Name (elem2.header.typeID)));
			++ind;
		}
	}
}
...
void	Do_CreateAnalyticalLink (void)
{
	API_ElemTypeID	typeID;
	API_Guid		startGuid;
	API_Guid		endGuid;
	API_Coord3D		startC3D;
	API_Coord3D		endC3D;
	API_Neig		startNeig;
	API_Neig		endNeig;
	if (!ClickAnElem ("Click the element for AnalyticalLink start. ", API_ZombieElemID, &startNeig, &typeID, &startGuid, &startC3D)) {
		WriteReport_Alert ("No element was clicked.");
		return;
	}

	if (!ClickAnElem ("Click the element for AnalyticalLink end. ", API_ZombieElemID, &endNeig, &typeID, &endGuid, &endC3D)) {
		WriteReport_Alert ("No element was clicked.");
		return;
	}
	...
	BNZeroMemory (&element, sizeof (API_Element));
	element.header.typeID = API_AnalyticalLinkID;
	...
	element.analyticalLink.startElementPartRef.elemGuid = startGuid;
	switch (startNeig.elemPartType) {
	case APINeigElemPart_None:		break;
	case APINeigElemPart_Edge:		element.analyticalLink.startElementPartRef.partRefType = API_AMPartRef_Edge; break;
	case APINeigElemPart_Vertex:	element.analyticalLink.startElementPartRef.partRefType = API_AMPartRef_BegVertex; break;
	case APINeigElemPart_Face:		element.analyticalLink.startElementPartRef.partRefType = API_AMPartRef_Surface; break;
	case APINeigElemPart_Subelem:	break;
	}
	element.analyticalLink.startElementPartRef.partId.edgeOrVertexIdx = startNeig.elemPartIndex;

	element.analyticalLink.endElementPartRef.elemGuid = endGuid;
	switch (endNeig.elemPartType) {
	case APINeigElemPart_None:		break;
	case APINeigElemPart_Edge:		element.analyticalLink.endElementPartRef.partRefType = API_AMPartRef_Edge; break;
	case APINeigElemPart_Vertex:	element.analyticalLink.endElementPartRef.partRefType = API_AMPartRef_BegVertex; break;
	case APINeigElemPart_Face:		element.analyticalLink.endElementPartRef.partRefType = API_AMPartRef_Surface; break;
	case APINeigElemPart_Subelem:	break;
	}
	element.analyticalLink.endElementPartRef.partId.edgeOrVertexIdx = endNeig.elemPartIndex;

	err = ACAPI_Element_Create (&element, &memo);
	if (err != NoError)
		ErrorBeep ("ACAPI_Element_Create (AnalyticalLink)", err);

	ACAPI_DisposeElemMemoHdls (&memo);
}		// Do_CreateAnalyticalLink
...
void	Do_CreateOpening (void)
{
	...
	if (!ClickAnElem ("Click a wall to place an opening", API_WallID, nullptr, &element.header.typeID, &wallGuid, &clickPoint)) {
		WriteReport_Alert ("Please click a wall");
		return;
	}
