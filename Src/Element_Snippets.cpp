...
static const API_Element	*actZone = nullptr;
...
static void		DumpConnections (const char *info, GS::Array<API_ConnectionGuidItem>* conns)
{
	char	s[1024];

	if (DBVERIFY (conns != nullptr) && !conns->IsEmpty ()) {
		strcpy (s, "");
		for (const API_ConnectionGuidItem& conn : *conns)
			sprintf (s, "%s  %s", s, APIGuidToString (conn.guid).ToCStr ().Get ());
	} else {
		sprintf (s, "---");
	}
...
{
	return ACAPI_Element_Delete ({ elemGuid });
}
...
void		Do_DumpZone (void)
{
	...
	if (!ClickAnElem ("Click a zone", API_ZoneID, nullptr, &element.header.typeID, &element.header.guid)) {
	...
	if (memo.gables != nullptr) {
		...
		for (i = 0; i < nGable; i++) {
			WriteReport ("  [%d] (%f, %f, %f, %f)",
							 ...
							 (*memo.gables)[i].d);
			if ((*memo.gables)[i].coords != nullptr) {
	...
	if (err == NoError) {
		for (const auto& pair : roomInfo.elementsGroupedByType) {
			WriteReport ("  %s: %d", ElemID_To_Name (*pair.key).ToCStr ().Get (),  pair.value->GetSize ());
		}
	}

	// -----------------------
	WriteReport ("Room-wall relations");

	for (UInt32 ii = 0; ii < roomInfo.wallPart.GetSize (); ++ii) {
		WriteReport ("wallPart[%d]", ii);
		WriteReport ("\t\troomEdge\t%d", roomInfo.wallPart[ii].roomEdge);
		WriteReport ("\t\ttBeg\t%5.2lf", roomInfo.wallPart[ii].tBeg);
		WriteReport ("\t\ttEnd\t%5.2lf", roomInfo.wallPart[ii].tEnd);
		WriteReport (" ");
	}

	// -----------------------
	WriteReport ("Room-beam relations");

	for (UInt32 ii = 0; ii < roomInfo.beamPart.GetSize (); ++ii) {
		WriteReport ("beamPart[%d]", ii);
		WriteReport ("\t\ttBeg\t%5.2lf", roomInfo.beamPart[ii].tBeg);
		WriteReport ("\t\ttEnd\t%5.2lf", roomInfo.beamPart[ii].tEnd);
		WriteReport (" ");
	}
...
void		Do_DumpWall (void)
{
	...
	if (!ClickAnElem ("Click a wall", API_WallID, nullptr, &element.header.typeID, &element.header.guid)) {
...
void		Do_DumpBeam (void)
{
	...
	if (!ClickAnElem ("Click a beam", API_BeamID, nullptr, &element.header.typeID, &element.header.guid)) {
	...
	DumpConnections ("  to BegPoint:", beamInfo.conBeg);
	DumpConnections ("  to EndPoint:", beamInfo.conEnd);
	DumpConnections ("  to Ref.Line:", beamInfo.conRef);
	DumpConnections ("  on the ends:", beamInfo.con);
	DumpConnections ("  crossing   :", beamInfo.conX);

	if (beamInfo.segmentRelations != nullptr) {
		UInt32 segmentIndex = 1;
		for (const API_BeamSegmentRelation& segmentInfo : *beamInfo.segmentRelations) {
			// -----------------------
			WriteReport ("Beam Segment #%u:", segmentIndex);

			// -----------------------
			WriteReport ("Connection polygon:");

			DumpPolygon (&element, 1, 0.0,
							segmentInfo.connPoly.nCoords, segmentInfo.connPoly.nSubPolys, segmentInfo.connPoly.nArcs,
							segmentInfo.coords, segmentInfo.pends, segmentInfo.parcs,
							true, true);

			// -----------------------
			WriteReport ("Connected beam segments:");

			DumpConnections ("  to BegPoint:", segmentInfo.conBeg);
			DumpConnections ("  to EndPoint:", segmentInfo.conEnd);
			DumpConnections ("  to Ref.Line:", segmentInfo.conRef);
			DumpConnections ("  on the ends:", segmentInfo.con);
			DumpConnections ("  crossing   :", segmentInfo.conX);
...
void		Do_DumpWindow (void)
{
	...
	if (!ClickAnElem ("Click a window", API_WindowID, nullptr, &element.header.typeID, &element.header.guid)) {
...
void		Do_DumpShell (void)
{
	...
	if (!ClickAnElem ("Click a shell", API_ShellID, nullptr, &element.header.typeID, &element.header.guid)) {
...
void		Do_NeigToCoord (void)
{
	...
	if (!ClickAnElem ("Click a point neig", API_ZombieElemID, &theNeig, &typeID, nullptr, &c2)) {
		WriteReport_Alert ("No element was clicked");
		return;
	}

	WriteReport ("%s #%s", UniStringToConstUTF8String (ElemID_To_Name (typeID)),
	             		   UniStringToConstCString (APIGuidToString (theNeig.guid)));
...
void		Do_GetBounds (void)
{
	...
	WriteReport ("%s GUID:%s", UniStringToConstUTF8String (ElemID_To_Name (element.header.typeID)),
	...
	if (fabs (box.zMax - box.zMin) > EPS) {
		...
		err = ACAPI_Element_GetDefaults (&element2, nullptr);
		if (err != NoError)
			return;
		...
		ACAPI_Element_Create (&element2, nullptr);

		GS::Array<API_Guid> elemGuids = { element2.header.guid };
		err = ACAPI_Element_Tool (elemGuids, APITool_SendBackward, nullptr);
		if (err != NoError)
		...
		element2.header.floorInd = element.header.floorInd;
		element2.header.layer = 1;
		element2.line.linePen.penIndex = 1;
...
void		Do_ShowSymbolHotspots (void)
{
	...
	if (!ClickAnElem ("Click a point neig", API_ZombieElemID, nullptr, &element.header.typeID, &element.header.guid)) {
		WriteReport_Alert ("No element was clicked");
		return;
	}
	...
	if (isLibPart && libInd != 0) {
		// reach back to the library part to check if a parameter is greyed out
	...
	if (err == NoError && !hotspotArray.IsEmpty ()) {
		...
		if (hotspotArray.GetSize () > 4) {
			// create a dimension line between 1st and the 4th hotspot
			API_Element		dim;
			API_ElementMemo	dimMemo;

			BNZeroMemory (&dim, sizeof (dim));
			BNZeroMemory (&dimMemo, sizeof (dimMemo));
			dim.header.typeID = API_DimensionID;
			ACAPI_Element_GetDefaults (&dim, nullptr);
			dim.dimension.dimAppear			= APIApp_Normal;
			dim.dimension.textPos			= APIPos_Above;
			dim.dimension.textWay			= APIDir_Parallel;
			dim.dimension.defStaticDim		= false;
			dim.dimension.usedIn3D			= false;
			dim.dimension.horizontalText	= false;

			dim.dimension.nDimElem			= 2;
			dimMemo.dimElems = reinterpret_cast<API_DimElem**> (BMhAllClear (dim.dimension.nDimElem * sizeof (API_DimElem)));
			for (Int32 i = 0; i < dim.dimension.nDimElem; ++i) {
				API_DimElem& dimElem = (*dimMemo.dimElems)[i];

				API_Neig	neig;
				API_Coord3D	coord;

				hotspotArray[i == 0 ? 1 : 3].Get (neig, coord);

				if (i == 0) {
					// the reference coordinate will be 1m below the first neig
					dim.dimension.refC.x = coord.x;
					dim.dimension.refC.y = coord.y - 1.0;
				} else if (i == 1) {
					// the direction is the vector from hotspot 1 to hotspot 4
					API_Coord	prevCoord;
					prevCoord.x = dim.dimension.refC.x;
					prevCoord.y = dim.dimension.refC.y + 1.0;

					dim.dimension.direction.x = prevCoord.x - coord.x;
					dim.dimension.direction.y = prevCoord.y - coord.y;
				}

				dimElem.base.loc.x			= coord.x;
				dimElem.base.loc.y			= coord.y;
				dimElem.base.base.line		= false;
				dimElem.base.base.special	= false;
				dimElem.base.base.inIndex	= neig.inIndex;
				dimElem.base.base.guid		= instanceElemGuid;

				dimElem.pos.x				= dimElem.base.loc.x;
				dimElem.pos.y				= dim.dimension.refC.y;

			}

			err = ACAPI_Element_Create (&dim, &dimMemo);
			if (err != NoError)
				ErrorBeep ("ACAPI_Element_Create (Dimension)", err);

			ACAPI_DisposeElemMemoHdls (&dimMemo);
		}
	}
...
void		Do_GetComponents (void)
{
	...
	if (!ClickAnElem ("Click an element to get its components", API_ZombieElemID, nullptr, &elemHead.typeID, &elemHead.guid)) {
		WriteReport_Alert ("No element was clicked");
		return;
	}
	...
	WriteReport ("%s GUID:%s", UniStringToConstUTF8String (ElemID_To_Name (elemHead.typeID)),
	             			   UniStringToConstCString (APIGuidToString (elemHead.guid)));
...
void		Do_SetToolBoxMode (void)
{
	...
	if (tboxInfo.typeID == API_ZombieElemID)
		...
		if (tboxInfo.variationID == APIVarId_Generic) {
			WriteReport ("Previous toolbox mode: %s", UniStringToConstUTF8String (ElemID_To_Name (tboxInfo.typeID)));
		} else {
			p = (char *) &tboxInfo.variationID;
			WriteReport ("Previous toolbox mode: %s  External \"%c%c%c%c\"",
			             UniStringToConstUTF8String (ElemID_To_Name (tboxInfo.typeID)),
			             p[0], p[1], p[2], p[3]);
		}
	}
	...
	tboxInfo.typeID = API_ZombieElemID;
	tboxInfo.variationID = APIVarId_Generic;
	if (ClickAnElem ("Click an element to set toolbox mode", API_ZombieElemID, nullptr, &tboxInfo.typeID, &guid)) {
	...
	err = ACAPI_Environment (APIEnv_SetToolBoxModeID, &tboxInfo, nullptr);
	if (err != NoError) {
	...
	if (tboxInfo.typeID == API_ZombieElemID)
		...
		if (tboxInfo.variationID == APIVarId_Generic) {
			WriteReport ("Current toolbox mode: %s", UniStringToConstUTF8String (ElemID_To_Name (tboxInfo.typeID)));
		} else {
			p = (char *) &tboxInfo.variationID;
			WriteReport ("Current toolbox mode: %s  External \"%c%c%c%c\"",
			             UniStringToConstUTF8String (ElemID_To_Name (tboxInfo.typeID)),
			             p[0], p[1], p[2], p[3]);
		}
	}
...
void		Do_ChangeSubtypeSettings (void)
{
	...
	if (err == APIERR_BADNAME)
		err = ACAPI_Attribute_Create (&attribute, nullptr);
	if (err != NoError) {
		ErrorBeep ("Error accessing layer \"ESYM layer\"", err);
		return;
	}
	const API_AttributeIndex attributeIndex = attribute.header.index;

	err = ACAPI_Environment (APIEnv_GetToolBoxInfoID, &toolboxInfo);
	if (err != NoError) {
...
void	Do_SplitPolyRoof (void)
{
	...
	if (!ClickAnElem ("Click a roof to modify", API_RoofID, nullptr, &element.header.typeID, &element.header.guid)) {
...
private:
	void Copy (const API_PrimElement* elem, const GSPtr inpar1, const GSPtr inpar2, const GSPtr inpar3) {
		...
		if (inpar1 != nullptr) {
			par1 = BMpAll (BMpGetSize (inpar1));	// buffer_overrun_reviewed_0
			if (par1 != nullptr)
				BNCopyMemory (par1, inpar1, BMpGetSize (inpar1));
		} else
			par1 = nullptr;
		if (inpar2 != nullptr) {
			par2 = BMpAll (BMpGetSize (inpar2));	// buffer_overrun_reviewed_0
			if (par2 != nullptr)
				BNCopyMemory (par2, inpar2, BMpGetSize (inpar2));
		} else
			par2 = nullptr;
		if (inpar3 != nullptr) {
			par3 = BMpAll (BMpGetSize (inpar3));	// buffer_overrun_reviewed_0
			if (par3 != nullptr)
	...
	void Init () {
		...
		par1 = par2 = par3 = nullptr;
	}

protected:
	GSErrCode	WriteHeader ();
	GSErrCode	WriteLinetype (API_AttributeIndex ltypeInd);
	GSErrCode	WriteFillMode (bool solid);
...
public:
	...
	inline void SetPen (short colorIndex) {
		prim.header.pen.penIndex = colorIndex;
	}
	...
	GSErrCode Draw () {
		...
		if (prim.header.pen.penIndex == 0 && !isRGBFill)
			return ret;
	...
	//		case API_PrimPictID:	ret = this->DrawPicture ();		break;
			case API_PrimPolyID:
				if (HatchLinesCollector::Instance ().GetState () == InBorder) {
					...
					prim.header.pen.penIndex = HatchLinesCollector::Instance ().GetFillBackgroundPen ();
					prim.poly.solid = true;
				}
				if (prim.header.pen.penIndex != 0)
...
GSErrCode	HatchLinesCollector::PushPrimElem (const PrimElem& inElem)
{
	...
	try {
		...
		//if (inElem.header.typeID == API_PrimPolyID)
			primElems.GetLast ().SetPen (border.fillPen.penIndex);
	}
...
{
	char	buff [128];
	static API_AttributeIndex	lastLay = -1;
	static short lastPen = -2;
	...
	if (prim.header.typeID == API_PrimPolyID && (prim.header.pen.penIndex == RoomPen || prim.header.pen.penIndex == RGBPen)) {
		...
		gOutput->Write (buff);
	} else if (lastPen != prim.header.pen.penIndex) {
		sprintf (buff, "  C %d\n", prim.header.pen.penIndex);
		gOutput->Write (buff);
		lastPen = prim.header.pen.penIndex;
	}
...
GSErrCode	PrimElem::WriteLinetype (API_AttributeIndex ltypeInd)
{
	char	buff [64];
	static API_AttributeIndex	lastLtype = -1;
...
GSErrCode	PrimElem::WriteFillMode (bool solid)
{
	char	buff [64];
...
GSErrCode	PrimElem::DrawPolygon ()
{
	...
	if (prim.header.pen.penIndex < 0)	// !!! polygon with plan background
		return NoError;
...
public:
	...
	{
		ACAPI_Goodies (APIAny_SetMasterLayoutOnLayoutID, (void*)(&databaseUnId), nullptr);
	}
...
{
	GSErrCode	ret = ACAPI_Database (APIDb_DrawOrderInitID, nullptr, nullptr);
	if (ret != NoError)
		return ret;
...
GSErrCode __ACENV_CALL Do_ExportPrimitives (const API_IOParams *ioParams)
{
	...
	if (err == NoError) {
		err = ACAPI_Database (APIDb_GetCurrentDatabaseID, &origDBInfo, nullptr);
		if (err == NoError && origDBInfo.masterLayoutUnId.elemSetId != APINULLGuid) {
			...
			showMasterBelow = masterLayoutInfo.showMasterBelow;
			if (masterLayoutInfo.customData != nullptr)
...
GSErrCode __ACENV_CALL Do_Import (const API_IOParams *ioParams)
{
	...
	if (ioParams->drawingGuid != APINULLGuid) {
		...
		if (err == NoError) {
			const API_Coord drawingPos = elem.drawing.pos;

			err = DeleteOneElement (ioParams->drawingGuid);
			if (err != NoError) {
				DBPrintf ("\t\tError %ld deleting drawing %s.\n", (Int32) err & 0x0000FFFF, APIGuidToString (elem.header.guid).ToCStr ().Get ());
				return err;
			}

			DBPrintf ("\t\tDrawing %s was deleted successfully.\n", APIGuidToString (elem.header.guid).ToCStr ().Get ());

			err = Do_CreateDrawingWithDummyElems (ioParams->drawingGuid, &drawingPos);
		}
	}
